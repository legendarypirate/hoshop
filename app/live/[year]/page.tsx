...
export default function LiveMonthPage() {
      ...
      data.forEach((order: any) => {
        if (order.paid_date) {
          ...
          if (orderDate.getFullYear() === yearNum) {
            monthSet.add(orderDate.getMonth() + 1);
          }
        }
      });
  ...
  return (
    <div className="container mx-auto py-10 px-4">
      <div className="mb-6">
        <Link href="/live" className="text-muted-foreground hover:text-foreground mb-2 inline-block">
          ← Буцах
        </Link>
        <h1 className="text-3xl font-bold">Live - {year}</h1>
